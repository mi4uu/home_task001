...
export const useGetFile = ({ id }: { id: number }) =>
  useQuery<iFileContent, Error>(['file', id], () => fetchFileById(id))
...
export const useGetRemoteApi = ({ url }: { url: string }) =>
  useMutation<iFile, Error>(['remoteApi', url], () => fetchRemoteApi(url))
...
export const useExtendFile = ({
  ...
  useMutation<iFile, Error>(['remoteApi', url, csv_id, csv_column, api_column], () =>
    extendFile({ url, csv_id: `${csv_id}`, csv_column, api_column })
  )
